 ...
 */
function scholarly_commerce_install_tasks_alter(&$tasks, $install_state) {
	...
	$tasks['scholarly_commerce_content'] = array(
 ...
 */
function scholarly_commerce_form_install_configure_form_alter(&$form, $form_state) {
	$form['site_information']['site_name']['#default_value'] = 'Scholarly';
    $form['#submit'][] = 'scholarly_commerce_install_configure_form_submit';
}

/**
 * Forms API submit for the site configuration form.
 */
function scholarly_commerce_install_configure_form_submit($form, &$form_state) {
	global $user;
	...
	drupal_set_message("Scholarly with Commerce Easy Installation was a Success!");

}

function scholarly_commerce_content(&$install_state) {
	$sql_file=dirname(__FILE__).'/scholarly_commerce.sql';
	$count=scholarly_commerce_import_sql($sql_file);
}

function scholarly_commerce_import_sql($filename){
